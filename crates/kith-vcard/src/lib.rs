...
//!
//! ```no_run
//! use kith_vcard::{parse, ParsedVcard};
//!
...
mod roundtrip_tests {
  ...
  use kith_core::{
    fact::{
      ...
      OrgMembershipValue, PhoneKind, PhoneValue, RecordingContext, RelationshipValue,
      SocialValue,
    },
  ...
  fn make_view(facts: Vec<FactValue>) -> ContactView {
    ...
    let subject    = Subject { subject_id, created_at: as_of, kind: SubjectKind::Person };
  ...
  where
    F: Fn(&FactValue) -> bool,
  {
    parsed.facts.iter().find(|f| predicate(&f.value)).map(|f| &f.value)
  }
  ...
  fn full_round_trip() {
    ...
    // Name
    let FactValue::Name(n) = find_fact(&parsed, |f| matches!(f, FactValue::Name(_))).unwrap()
    else { panic!("no Name fact") };
    assert_eq!(n.full,   "Alice Smith");
    assert_eq!(n.family, Some("Smith".to_string()));
    assert_eq!(n.given,  Some("Alice".to_string()));

    // Email
    let FactValue::Email(e) = find_fact(&parsed, |f| matches!(f, FactValue::Email(_))).unwrap()
    else { panic!("no Email fact") };
    assert_eq!(e.address,    "alice@example.com");
    assert_eq!(e.preference, 1);

    // Phone
    let FactValue::Phone(p) = find_fact(&parsed, |f| matches!(f, FactValue::Phone(_))).unwrap()
    else { panic!("no Phone fact") };
    assert_eq!(p.number, "+15555551234");
    assert_eq!(p.kind,   PhoneKind::Cell);

    // Address
    let FactValue::Address(a) = find_fact(&parsed, |f| matches!(f, FactValue::Address(_))).unwrap()
    else { panic!("no Address fact") };
    assert_eq!(a.street,   Some("123 Main St".to_string()));
    assert_eq!(a.locality, Some("Springfield".to_string()));

    // OrgMembership
    let FactValue::OrgMembership(o) = find_fact(&parsed, |f| matches!(f, FactValue::OrgMembership(_))).unwrap()
    else { panic!("no OrgMembership fact") };
    assert_eq!(o.org_name, "Acme Corp");
    assert_eq!(o.title,    Some("Engineer".to_string()));
    assert_eq!(o.role,     Some("IC".to_string()));

    // Note
    let FactValue::Note(note) = find_fact(&parsed, |f| matches!(f, FactValue::Note(_))).unwrap()
    else { panic!("no Note fact") };
    assert_eq!(note, "First met at conference.");

    // Social
    let FactValue::Social(s) = find_fact(&parsed, |f| matches!(f, FactValue::Social(_))).unwrap()
    else { panic!("no Social fact") };
    assert_eq!(s.platform, "Twitter");
    assert_eq!(s.handle,   "@alice");

    // Relationship
    let FactValue::Relationship(r) = find_fact(&parsed, |f| matches!(f, FactValue::Relationship(_))).unwrap()
    else { panic!("no Relationship fact") };
