...
use kith_core::{
  fact::{
    Confidence, EmailValue, ContactLabel, FactValue, NameValue, NewFact,
    RecordingContext,
  },
  store::ContactStore,
  subject::SubjectKind,
...
async fn record_multiple_facts() {
  ...
  s.record_fact(email_fact(subject.subject_id, "alice@example.com")).await.unwrap();
  s.record_fact(email_fact(subject.subject_id, "alice@work.example.com")).await.unwrap();
...
async fn fact_confidence_and_source_roundtrip() {
  ...
  let facts = s.get_facts(subject.subject_id, None, false).await.unwrap();
  let rf    = facts.into_iter().find(|rf| rf.fact.fact_id == fact.fact_id).unwrap();
...
async fn supersede_marks_old_inactive() {
  ...
  let old = s.record_fact(email_fact(subject.subject_id, "old@example.com")).await.unwrap();
  ...
  let old_rf = all.iter().find(|rf| rf.fact.fact_id == old.fact_id).unwrap();
  assert!(matches!(old_rf.status, kith_core::lifecycle::FactStatus::Superseded { by, .. } if by == new_fact.fact_id));
}
...
async fn supersede_already_superseded_errors() {
  ...
  let old = s.record_fact(email_fact(subject.subject_id, "a@example.com")).await.unwrap();
  s.supersede(old.fact_id, email_fact(subject.subject_id, "b@example.com")).await.unwrap();
...
async fn supersede_nonexistent_fact_errors() {
  ...
  let err = s
    .supersede(Uuid::new_v4(), email_fact(subject.subject_id, "a@example.com"))
    .await
    .unwrap_err();
...
async fn retract_removes_from_active_view() {
  ...
  let fact = s.record_fact(email_fact(subject.subject_id, "gone@example.com")).await.unwrap();
  let ret  = s.retract(fact.fact_id, Some("wrong address".into())).await.unwrap();
...
async fn retract_already_retracted_errors() {
  ...
  let fact = s.record_fact(email_fact(subject.subject_id, "x@example.com")).await.unwrap();
...
async fn materialize_active_facts() {
  ...
  let email = s.record_fact(email_fact(subject.subject_id, "alice@example.com")).await.unwrap();
  let stale = s.record_fact(email_fact(subject.subject_id, "old@example.com")).await.unwrap();
  s.supersede(stale.fact_id, email_fact(subject.subject_id, "newer@example.com")).await.unwrap();

  let view = s.materialize(subject.subject_id, None).await.unwrap().unwrap();
  ...
  let ids: Vec<_> = view.active_facts.iter().map(|rf| rf.fact.fact_id).collect();
...
async fn search_by_text() {
  ...
  s.record_fact(email_fact(alice.subject_id, "alice@example.com")).await.unwrap();
  s.record_fact(email_fact(bob.subject_id, "bob@example.com")).await.unwrap();
