...
pub fn diff(
  ...
  let active: Vec<&kith_core::lifecycle::ResolvedFact> = view.active_facts.iter().collect();

  let mut new_facts:     Vec<NewFact>                   = vec![];
  let mut supersessions: Vec<(Uuid, NewFact)>            = vec![];
  // Track which active fact IDs were matched.
  let mut matched_active: std::collections::HashSet<Uuid> = std::collections::HashSet::new();
  ...
  Ok(DiffResult { new_facts, supersessions, retractions })
}
...
fn fact_matches(incoming: &FactValue, existing: &FactValue) -> bool {
  ...
  match (incoming, existing) {
    ...
    (Email(a), Email(b)) => a.address.to_lowercase() == b.address.to_lowercase(),

    // Key: number (stripped of whitespace/dashes).
    (Phone(a), Phone(b)) => normalize_phone(&a.number) == normalize_phone(&b.number),
    ...
    (GroupMembership(a), GroupMembership(b)) => match (&a.group_id, &b.group_id) {
...
mod tests {
  ...
  fn unchanged_contact_empty_result() {
    ...
    let active_facts: Vec<ResolvedFact> = r1.new_facts.into_iter().map(|f| ResolvedFact {
      fact: Fact {
        fact_id:           Uuid::new_v4(),
        subject_id:        id,
        value:             f.value,
    ...
    let view = ContactView {
      subject:      Subject { subject_id: id, created_at: ts, kind: SubjectKind::Person },
      as_of:        ts,
      active_facts,
    };

    let r2 = diff(vcard, id, SRC, Some(&view)).unwrap();
    assert!(r2.new_facts.is_empty(),     "unexpected new facts: {:?}", r2.new_facts.len());
  ...
  fn initial_view(vcard: &str, id: Uuid) -> ContactView {
    ...
    let active_facts = r.new_facts.into_iter().map(|f| ResolvedFact {
      fact: Fact {
        fact_id:           Uuid::new_v4(),
        subject_id:        id,
        value:             f.value,
  ...
  fn email_change_is_supersession() {
    ...
    assert!(result.new_facts.is_empty(),   "unexpected new_facts: {}", result.new_facts.len());
    assert!(result.retractions.is_empty(), "unexpected retractions: {}", result.retractions.len());
  }
  ...
  fn new_phone_is_new_fact() {
    ...
    let phones: Vec<_> = result.new_facts.iter()
      .filter(|f| matches!(f.value, FactValue::Phone(_)))
      .collect();
    assert_eq!(phones.len(), 1,             "expected one new phone");
    assert!(result.supersessions.is_empty(), "unexpected supersessions: {}", result.supersessions.len());
    assert!(result.retractions.is_empty(),   "unexpected retractions: {}", result.retractions.len());
  }
  ...
  fn full_contact_round_trip() {
    ...
    assert!(r2.supersessions.is_empty(), "sup={}", r2.supersessions.len());
